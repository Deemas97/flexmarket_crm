...
class EmployeesActionsController extends ControllerAbstract
{
    ...
    public function apiCreate(): ControllerResponseInterface
    {
        ...
        // Валидация
        if (empty($data['f']) || empty($data['i']) || empty($data['email']) || empty($data['password'])) {
        ...
        $emailCheck = $db->query("SELECT id FROM employees WHERE email = '{$db->escape($data['email'])}'");
        if (!empty($emailCheck)) {
            return $this->initJsonResponse();
        }
        ...
        $result = $db->query("INSERT INTO employees ($columns) VALUES ($values)");
    ...
    public function apiUpdate(int $id): ControllerResponseInterface
    {
        ...
        // Валидация
        if (empty($data['f']) || empty($data['i']) || empty($data['email'])) {
            return $this->initJsonResponse();
        }

        // Проверка существования пользователя
        $user = $db->query("SELECT id FROM employees WHERE id = {$id} LIMIT 1");
        if (empty($user)) {
            return $this->initJsonResponse();
        }
        ...
        $result = $db->query("UPDATE employees SET " . implode(', ', $updateParts) . " WHERE id = {$id}");
        ...
        if ($id === $this->auth->getUserId()) {
            $this->auth->setUserName($data['i'] . ' ' . $data['f']);
    ...
    public function apiChangePassword(int $id): ControllerResponseInterface
    {
        ...
        $user = $db->query("SELECT salt FROM employees WHERE id = {$id} LIMIT 1");
        if (empty($user)) {
            return $this->initJsonResponse();
        }
        ...
        $result = $db->query("UPDATE employees SET password_hash = '{$db->escape($newPasswordHash)}' WHERE id = {$id}");

        if ($result === false) {
            return $this->initJsonResponse();
        }

        $this->redirect('/employee/' . $id . '/edit');
