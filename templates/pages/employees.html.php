                        ...
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td><?= $this->escape($employee['id']) ?></td>
                                        <td>
                                            <?= $this->escape($employee['f']) ?> 
                                            <?= $this->escape($employee['i']) ?>
                                            <?= isset($employee['o']) ? $this->escape($employee['o']) : '' ?>
                                        </td>
                                        <td>
                                            <?php if ($employee['role_id'] === 'admin'): ?>
                                                <span class="badge bg-danger">Администратор</span>
                                            <?php else: ?>
                                                <span class="badge bg-primary">Менеджер</span>
                                            <?php endif; ?>
                                        </td>
                                        <td><?= $this->escape($employee['email']) ?></td>
                                        <td>
                                        ...
                                        <?php if ($data['user_session']['role'] === 'admin'): ?>
                                            ...
                                            <div class="btn-group btn-group-sm">
                                                <a href="/employee/<?= $employee['id'] ?>/edit" class="btn btn-outline-primary" title="Редактировать">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <button class="btn btn-outline-danger delete-user" data-id="<?= $employee['id'] ?>" title="Удалить">
...
<div class="modal fade" id="addUserModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form id="addUserForm" action=".local:1138/api/employee/create" method="POST">
...
$(document).ready(function() {
    ...
    $('.delete-user').click(function() {
        ...
        $('#confirmDelete').off('click').on('click', function() {
            $.ajax({
                url: '/api/employee/' + userId + '/delete',
                method: 'DELETE',
                success: function() {
                    location.reload();
                },
